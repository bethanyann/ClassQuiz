...
switch ($action) {
    ...
    case 'createNewQuiz': //coming in from the admin_quiz_list page if a new quiz is created               
        ...
        //make sure that the chapter number does not exist for this class already
        $doesChapterExist = ValidateQuizChapter($courseID, $chapterNumber);
        if (!$doesChapterExist) {
            ...
            //the chapter exists already and needs to be sent back to the page
            $error_message = "A quiz exists for that Chapter already.";
            include'../view/admin_create_quiz.php';
        }
        break;
    case 'saveQuestion': //this action will be hit every time the user wants to save a question/answer set
        ...
        if (isset($quiz)) { //if the quiz has been started
            ...
            //get question type
            $qt = filter_var($_POST['questionType']);
            ...
            if ($questionCounter <= $totalNumQuestions) {   //increment the counter
    ...
    case 'saveQuiz' : //permanently saves the quiz, questions and answers to the DB
        ...
        if ($results) {   //if the quiz successfully saved, get the new list of quizzes
            //destroy quiz out of session
            $_SESSION['quiz']=null;
            //get list of quizzes to display back to user
    ...
    case 'viewQuiz' :
        ...
        $quiz = new Quiz();
        $quiz->quizID = $quizID;
        $quiz->courseID = $_SESSION['courseID'];
    ...
    case 'deleteQuiz' :
        ...
        if ($success) {
            //get the updated list of quizzes
            $listOfQuizzes = GetCourseQuizzes($quiz->courseID);
            include '../view/admin_quiz_list.php';
        }
        break;
